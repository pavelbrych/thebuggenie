			...
			case TBGWorkflowTransitionAction::ACTION_ASSIGN_ISSUE:
				...
				<td id="workflowtransitionaction_<?php echo $action->getID(); ?>_description" style="padding: 2px;">
					<?php if ($action->getActionType() == TBGWorkflowTransitionAction::ACTION_SET_STATUS): ?>
						<?php echo __('Set status to %status%', array('%status%' => '<span id="workflowtransitionaction_'.$action->getID().'_value" style="font-weight: bold;">' . (($action->getTargetValue()) ? \caspar\core\Caspar::factory()->manufacture('\thebuggenie\entities\Status', (int) $action->getTargetValue())->getName() : __('Status provided by user')) . '</span>')); ?>
					<?php elseif ($action->getActionType() == TBGWorkflowTransitionAction::ACTION_SET_PRIORITY): ?>
						<?php echo __('Set priority to %priority%', array('%priority%' => '<span id="workflowtransitionaction_'.$action->getID().'_value" style="font-weight: bold;">' . (($action->getTargetValue()) ? \caspar\core\Caspar::factory()->manufacture('\thebuggenie\entities\Priority', (int) $action->getTargetValue())->getName() : __('Priority provided by user')) . '</span>')); ?>
					...
					<?php elseif ($action->getActionType() == TBGWorkflowTransitionAction::ACTION_SET_RESOLUTION): ?>
						<?php echo __('Set resolution to %resolution%', array('%resolution%' => '<span id="workflowtransitionaction_'.$action->getID().'_value" style="font-weight: bold;">' . (($action->getTargetValue()) ? \caspar\core\Caspar::factory()->manufacture('\thebuggenie\entities\Resolution', (int) $action->getTargetValue())->getName() : __('Resolution provided by user')) . '</span>')); ?>
					<?php elseif ($action->getActionType() == TBGWorkflowTransitionAction::ACTION_SET_REPRODUCABILITY): ?>
						<?php echo __('Set reproducability to %reproducability%', array('%reproducability%' => '<span id="workflowtransitionaction_'.$action->getID().'_value" style="font-weight: bold;">' . (($action->getTargetValue()) ? \caspar\core\Caspar::factory()->manufacture('\thebuggenie\entities\Reproducability', (int) $action->getTargetValue())->getName() : __('Reproducability provided by user')) . '</span>')); ?>
