<?php
	...
	if ($csp_user->hasPageAccess('project_timeline', $project->getID()) || $csp_user->hasPageAccess('project_allpages', $project->getID()))
	{
		$csp_response->addFeed(make_url('project_timeline', array('project_key' => $project->getKey(), 'format' => 'rss')), __('"%project_name%" project timeline', array('%project_name%' => $project->getName())));
	}
...
<div class="rounded_box <?php if (!($project->isIssuelistVisibleInFrontpageSummary() && count($project->getVisibleIssuetypes()))): ?>invisible <?php else: ?> white borderless <?php endif; ?>project_strip">
	<div style="float: left; font-weight: normal;">
		...
		<b class="project_name"><?php echo link_tag(make_url('project_dashboard', array('project_key' => $project->getKey())), '<span id="project_name_span">'.$project->getName()."</span>"); ?> <?php if ($project->usePrefix()): ?>(<?php echo mb_strtoupper($project->getPrefix()); ?>)<?php endif; ?></b><?php if ($csp_user->canEditProjectDetails($project)): ?>&nbsp;&nbsp;<span class="faded_out"><?php echo javascript_link_tag(__('Edit project'), array('onclick' => "TBG.Main.Helpers.Backdrop.show('".make_url('get_partial_for_backdrop', array('key' => 'project_config', 'project_id' => $project->getID()))."');", 'style' => 'font-size: 12px;')); ?></span><?php endif; ?><br>
	...
	<nav>
		<?php if ($csp_user->canSearchForIssues() && ($csp_user->hasPageAccess('project_issues', $project->getID()) || $csp_user->hasPageAccess('project_allpages', $project->getID()))): ?>
			<?php echo link_tag(make_url('project_open_issues', array('project_key' => $project->getKey())), __('Issues')); ?>&nbsp;&nbsp;&nbsp;&nbsp;
		<?php endif; ?>
		<?php if ($csp_user->hasPageAccess('project_roadmap', $project->getID()) || $csp_user->hasPageAccess('project_allpages', $project->getID())): ?>
			<?php echo link_tag(make_url('project_roadmap', array('project_key' => $project->getKey())), __('Show roadmap')); ?>&nbsp;&nbsp;&nbsp;&nbsp;
		<?php endif; ?>
		<?php \caspar\core\Event::createNew('core', 'project_overview_item_links', $project)->trigger(); ?>
		<?php if ($csp_user->canReportIssues($project)): ?>
