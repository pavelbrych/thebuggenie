...
<?php if ($csp_user->hasProjectPageAccess('project_dashboard', $selected_project->getID())): ?>
	...
	<?php if (!($submenu) && $csp_response->getPage() == 'project_dashboard' && $csp_user->canEditProjectDetails($selected_project)): ?>
		<ul class="simple_list">
			<li><?php echo javascript_link_tag('<span>'.__('Customize').'</span>', array('title' => __('Customize'), 'onclick' => "TBG.Main.Helpers.Backdrop.show('".make_url('get_partial_for_backdrop', array('key' => 'dashboard_config', 'tid' => $selected_project->getID(), 'target_type' => TBGDashboardView::TYPE_PROJECT, 'previous_route'))."');")); ?></li>
		</ul>
	<?php endif; ?>
<?php endif; ?>
...
<?php if ($csp_user->canEditProjectDetails($selected_project)): ?>
	...
	<?php if (!($submenu) && $csp_response->getPage() == 'project_settings'): ?>
		...
		<ul class="simple_list" id="project_config_menu">
			...
			<?php \caspar\core\Event::createNew('core', 'config_project_tabs')->trigger(array('selected_tab' => $selected_tab)); ?>
		</ul>
	<?php endif; ?>
<?php endif; ?>
