<?php
	...
	$csp_response->addBreadcrumb(__('Timeline'), null, tbg_get_breadcrumblinks('project_summary', $selected_project));
	$csp_response->setTitle(__('"%project_name%" project timeline', array('%project_name%' => $selected_project->getName())));
...
?>
			...
			<?php if (count($recent_activities) > 0): ?>
				...
				<?php echo javascript_link_tag(image_tag('action_add_small.png').__('Show more'), array('class' => 'button button-silver', 'style' => 'padding: 3px 6px !important; font-size: 1.1em; font-weight: normal;' , 'onclick' => "TBG.Project.Timeline.update('".make_url('project_timeline', array('project_key' => $selected_project->getKey()))."');", 'id' => 'timeline_more_link')); ?>
			<?php endif; ?>
		</td>
	</tr>
</table>
