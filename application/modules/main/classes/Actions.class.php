<?php 
	...
	class Actions extends \caspar\core\Actions
	{
		...
		public function preExecute(Request $request, $action)
		{
			try {
				...
				elseif ($project_id = (int) $request->getParameter('project_id'))
					$this->selected_project = Context::factory()->manufacture('\\thebuggenie\\entities\\Project', $project_id);
		...
		public function runViewIssue(Request $request)
		{
			...
			$uploaded = Context::getMessageAndClear('issue_file_uploaded');
			
			if ($request->isMethod(Request::POST) && $issue instanceof \TBGIssue && $request->hasParameter('issue_action'))
			{
						...
						if ($issue->hasUnsavedChanges())
						{
							if (!$issue->hasMergeErrors())
							{
								try
								{
									...
									$issue->save();
									Context::setMessage('issue_saved', true);
			...
			elseif (Context::hasMessage('issue_error'))
			{
				$this->error = Context::getMessageAndClear('issue_error');
			}
			elseif (Context::hasMessage('issue_message'))
			{
				$this->issue_message = Context::getMessageAndClear('issue_message');
			}
			...
			if (count($issues))
			{
				foreach ($issues as $starred_issue)
				{
					...
					{
						$issuelist[$starred_issue->getID()] = array('url' => Context::getRouting()->generate('viewissue', array('project_key' => $this->selected_project->getKey(), 'issue_no' => $starred_issue->getFormattedIssueNo())), 'title' => $starred_issue->getFormattedTitle(true, true));
					}
				}
			}
			...
			if (array_key_exists('viewissue_list', $_SESSION) && is_array($_SESSION['viewissue_list'])) {
				foreach ($_SESSION['viewissue_list'] as $k => $i_id)
				{
					try
					{
						...
						array_unshift($issuelist, array('url' => Context::getRouting()->generate('viewissue', array('project_key' => $an_issue->getProject()->getKey(), 'issue_no' => $an_issue->getFormattedIssueNo())), 'title' => $an_issue->getFormattedTitle(true, true)));
					}
					catch (Exception $e)
					{
		...
		public function runMoveIssue(Request $request)
		{
			...
			if ($issue_id = $request->getParameter('issue_id'))
			{
				try
				{
					$issue = Context::factory()->TBGIssue($issue_id);
				}
				catch (Exception $e) { }
			}
			if ($project_id = $request->getParameter('project_id'))
			{
				try
				{
					$project = Context::factory()->TBGProject($project_id);
				}
				catch (Exception $e) { }
			}
			...
			if (!$issue instanceof \TBGIssue)
			{
				return $this->return404(Context::getI18n()->__('Cannot find the issue specified'));
			}

			if (!$project instanceof TBGProject)
			{
				return $this->return404(Context::getI18n()->__('Cannot find the project specified'));
			}
			...
			if ($issue->getProject()->getID() != $project->getID())
			{
				...
				$issue->save();
				Context::setMessage('issue_message', Context::getI18n()->__('The issue was moved'));
			}
			else
			{
				Context::setMessage('issue_error', Context::getI18n()->__('The issue was not moved, since the project is the same'));
			}
			
			return $this->forward(Context::getRouting()->generate('viewissue', array('project_key' => $project->getKey(), 'issue_no' => $issue->getFormattedIssueNo())));
		}
		...
		public function runIndex(Request $request)
		{
			...
			$this->show_project_config_link = Context::getUser()->canAccessConfigurationPage(\TBGSettings::CONFIGURATION_SECTION_PROJECTS);
		 ...
		 * @param Request $request
		 */
		public function runDashboardSave(Request $request)
		{
			...
			try
			{
				if ($request->isAjaxCall() || $request->getRequestedFormat() == 'json')
				{
		...
		public function runClientDashboard(Request $request)
		{
			...
			try
			{
				$this->client = Context::factory()->TBGClient($request->getParameter('client_id'));
		...
		public function runTeamDashboard(Request $request)
		{
			try
			{
				$this->team = Context::factory()->TBGTeam($request->getParameter('team_id'));
		...
		public function runLogout(Request $request)
		{
			...
			Context::logout();
			$this->forward(Context::getRouting()->generate(\TBGSettings::getLogoutReturnRoute()));
		}
		...
		public function runDoLogin(Request $request)
		{
			...
			$forward_url = Context::getRouting()->generate('home');

			$openid = new LightOpenID(Context::getRouting()->generate('login_page', array(), false));
			if (!$openid->mode && $request->isMethod(Request::POST) && $request->hasParameter('openid_identifier'))
			{
			...
			{
				$this->error = Context::getI18n()->__("OpenID authentication cancelled");
			} 
			elseif ($openid->mode)
			{
				try
				{
					if ($openid->validate())
					{
						...
						if ($user instanceof TBGUser)
						{
							...
							return $this->forward(Context::getRouting()->generate('account'));
						}
						else
						{
							$this->error = Context::getI18n()->__("Didn't recognize this OpenID. Please log in using your username and password, associate it with your user account in your account settings and try again.");
						}
					}
					else
					{
						$this->error = Context::getI18n()->__("Could not validate against the OpenID provider");
					}
				}
				catch (Exception $e)
				{
			...
			elseif ($request->getMethod() == Request::POST)
			{
				try
				{
					if ($request->hasParameter('tbg3_username') && $request->hasParameter('tbg3_password') && $request->getParameter('tbg3_username') != '' && $request->getParameter('tbg3_password') != '')
					{
						...
						else
						{
							...
							{
								$forward_url = $request->getParameter('tbg3_referer', Context::getRouting()->generate('dashboard'));
							}
							else
							{
								$forward_url = Context::getRouting()->generate(\TBGSettings::get('returnfromlogin'));
							}
						}
					}
					else
		...
		public function runActivate(Request $request)
		{
			...
			$this->forward(Context::getRouting()->generate('login_page'));
		}
			...
			if ($request->isMethod(Request::POST) && $request->hasParameter('mode'))
			{
				switch ($request->getParameter('mode'))
				{
					...
					case 'settings':
						...
						break;
					case 'module':
						foreach (Context::getModules() as $module_name => $module)
						{
							if ($request->getParameter('target_module') == $module_name && $module->hasAccountSettings())
							{
								if ($module->postAccountSettings($request))
								{
									return $this->renderJSON(array('failed' => false, 'title' => Context::getI18n()->__('Settings saved'), 'content' => ''));
								}
								else
								{
									return $this->renderJSON(array('failed' => true, 'error' => Context::getI18n()->__('An error occured')));
								}
							}
						}
						break;
		...
		public function runAccountChangePassword(Request $request)
		{
			...
			if ($request->isMethod(Request::POST))
			{
				...
				{
					return $this->renderJSON(array('failed' => true, 'error' => Context::getI18n()->__('Please enter a new password')));
				}
		...
		protected function _loadSelectedProjectAndIssueTypeFromRequestForReportIssueAction(Request $request)
		{
			try
			{
				...
				elseif ($project_id = $request->getParameter('project_id'))
					$this->selected_project = Context::factory()->TBGProject($project_id);
			}
			catch (Exception $e) {}
			...
			if (!$this->selected_issuetype instanceof \TBGIssuetype)
			{
				...
				if ($this->issuetype_id)
				{
					try
					{
						$this->selected_issuetype = Context::factory()->TBGIssuetype($this->issuetype_id);
					}
					catch (Exception $e) {}
				}
			}
		...
		protected function _postIssueValidation(Request $request, &$errors, &$permission_errors)
		{
			...
			if (empty($errors))
			{
				...
				if ($edition_id = (int) $request->getParameter('edition_id'))
					$this->selected_edition = Context::factory()->TBGEdition($edition_id);
				if ($build_id = (int) $request->getParameter('build_id'))
					$this->selected_build = Context::factory()->TBGBuild($build_id);
				if ($component_id = (int) $request->getParameter('component_id'))
					$this->selected_component = Context::factory()->TBGComponent($component_id);
				...
				if ($category_id = (int) $request->getParameter('category_id'))
					$this->selected_category = Context::factory()->TBGCategory($category_id);

				if ($status_id = (int) $request->getParameter('status_id'))
					$this->selected_status = Context::factory()->TBGStatus($status_id);

				if ($reproducability_id = (int) $request->getParameter('reproducability_id'))
					$this->selected_reproducability = Context::factory()->TBGReproducability($reproducability_id);

				if ($resolution_id = (int) $request->getParameter('resolution_id'))
					$this->selected_resolution = Context::factory()->TBGResolution($resolution_id);

				if ($severity_id = (int) $request->getParameter('severity_id'))
					$this->selected_severity = Context::factory()->TBGSeverity($severity_id);

				if ($priority_id = (int) $request->getParameter('priority_id'))
					$this->selected_priority = Context::factory()->TBGPriority($priority_id);
		...
		public function runReportIssue(Request $request)
		{
			...
			$this->forward403unless(Context::getCurrentProject() instanceof TBGProject && Context::getCurrentProject()->hasAccess() && Context::getUser()->canReportIssues(Context::getCurrentProject()));
			...
			if ($request->isMethod(Request::POST))
			{
				if ($this->_postIssueValidation($request, $errors, $permission_errors))
				{
					try
					{
						...
						{
							$this->forward(Context::getRouting()->generate('viewissue', array('project_key' => $issue->getProject()->getKey(), 'issue_no' => $issue->getFormattedIssueNo())), 303);
						}
						else
						{
		...
		public function _setFieldFromRequest(Request $request)
		{
		...
		public function runIssueSetField(Request $request)
		{
			...
			{
				case 'description':
					if (!$issue->canEditDescription()) return $this->renderJSON(array('changed' => false, 'error' => Context::getI18n()->__('You do not have permission to perform this action')));
				...
				case 'reproduction_steps':
					if (!$issue->canEditReproductionSteps()) return $this->renderJSON(array('changed' => false, 'error' => Context::getI18n()->__('You do not have permission to perform this action')));
					...
					break;
				case 'title':
					if (!$issue->canEditTitle()) return $this->renderJSON(array('changed' => false, 'error' => Context::getI18n()->__('You do not have permission to perform this action')));
					
					if ($request->getParameter('value') == '')
					{
						return $this->renderJSON(array('changed' => false, 'failed' => true, 'error' => Context::getI18n()->__('You have to provide a title')));
					}
					else
					{
					...
					break;
				case 'percent':
					if (!$issue->canEditPercentage()) return $this->renderJSON(array('changed' => false, 'error' => Context::getI18n()->__('You do not have permission to perform this action')));
					...
					break;
				case 'estimated_time':
					if (!$issue->canEditEstimatedTime()) return $this->renderJSON(array('changed' => false, 'error' => Context::getI18n()->__('You do not have permission to perform this action')));
					
					if ($request->getParameter('estimated_time') != Context::getI18n()->__('Enter your estimate here') && $request->getParameter('estimated_time'))
					{
				...
				case 'posted_by':
				case 'assigned_to':
					if ($request->getParameter('field') == 'owned_by' && !$issue->canEditOwnedBy()) return $this->renderJSON(array('changed' => false, 'error' => Context::getI18n()->__('You do not have permission to perform this action')));
					elseif ($request->getParameter('field') == 'posted_by' && !$issue->canEditPostedBy()) return $this->renderJSON(array('changed' => false, 'error' => Context::getI18n()->__('You do not have permission to perform this action')));
					elseif ($request->getParameter('field') == 'assigned_to' && !$issue->canEditAssignedTo()) return $this->renderJSON(array('changed' => false, 'error' => Context::getI18n()->__('You do not have permission to perform this action')));
							...
							if (in_array($request->getParameter('identifiable_type'), array(TBGIdentifiableClass::TYPE_USER, TBGIdentifiableClass::TYPE_TEAM)))
							{
								switch ($request->getParameter('identifiable_type'))
								{
									case TBGIdentifiableClass::TYPE_USER:
										$identified = Context::factory()->TBGUser($request->getParameter('value'));
										break;
									case TBGIdentifiableClass::TYPE_TEAM:
										$identified = Context::factory()->TBGTeam($request->getParameter('value'));
										break;
								}
					...
					break;
				case 'spent_time':
					if (!$issue->canEditSpentTime()) return $this->renderJSON(array('changed' => false, 'error' => Context::getI18n()->__('You do not have permission to perform this action')));
					
					if ($request->getParameter('spent_time') != Context::getI18n()->__('Enter time spent here') && $request->getParameter('spent_time'))
					{
				...
				case 'pain_effect':
					if ($request->getParameter('field') == 'category' && !$issue->canEditCategory()) return $this->renderJSON(array('changed' => false, 'error' => Context::getI18n()->__('You do not have permission to perform this action')));
					elseif ($request->getParameter('field') == 'resolution' && !$issue->canEditResolution()) return $this->renderJSON(array('changed' => false, 'error' => Context::getI18n()->__('You do not have permission to perform this action')));
					elseif ($request->getParameter('field') == 'severity' && !$issue->canEditSeverity()) return $this->renderJSON(array('changed' => false, 'error' => Context::getI18n()->__('You do not have permission to perform this action')));
					elseif ($request->getParameter('field') == 'reproducability' && !$issue->canEditReproducability()) return $this->renderJSON(array('changed' => false, 'error' => Context::getI18n()->__('You do not have permission to perform this action')));
					elseif ($request->getParameter('field') == 'priority' && !$issue->canEditPriority()) return $this->renderJSON(array('changed' => false, 'error' => Context::getI18n()->__('You do not have permission to perform this action')));
					elseif ($request->getParameter('field') == 'milestone' && !$issue->canEditMilestone()) return $this->renderJSON(array('changed' => false, 'error' => Context::getI18n()->__('You do not have permission to perform this action')));
					elseif ($request->getParameter('field') == 'issuetype' && !$issue->canEditIssuetype()) return $this->renderJSON(array('changed' => false, 'error' => Context::getI18n()->__('You do not have permission to perform this action')));
					elseif ($request->getParameter('field') == 'status' && !$issue->canEditStatus()) return $this->renderJSON(array('changed' => false, 'error' => Context::getI18n()->__('You do not have permission to perform this action')));
					elseif (in_array($request->getParameter('field'), array('pain_bug_type', 'pain_likelihood', 'pain_effect')) && !$issue->canEditUserPain()) return $this->renderJSON(array('changed' => false, 'error' => Context::getI18n()->__('You do not have permission to perform this action')));
					...
					try
					{
						...
						if ($request->hasParameter($parameter_id_name)) //$request->getParameter('field') == 'pain_bug_type')
						{
							...
							if ($parameter_id !== 0)
							{
								$is_valid = ($is_pain) ? in_array($parameter_id, array_keys(\TBGIssue::getPainTypesOrLabel($parameter_name))) : ($parameter_id == 0 || (($parameter = Context::factory()->$lab_function_name($parameter_id)) instanceof TBGIdentifiableClass));
							}
							if ($parameter_id == 0 || ($parameter_id !== 0 && $is_valid))
							{
								...
								else
								{
									...
									if ($classname == '\TBGIssuetype')
									{
										...
										$field['src'] = htmlspecialchars(Context::getTBGPath() . 'themes/' . \TBGSettings::getThemeName() . '/' . $issue->getIssuetype()->getIcon() . '_small.png');
									}
					...
					$this->getResponse()->setHttpStatus(400);
					return $this->renderJSON(array('error' => Context::getI18n()->__('No valid field value specified')));
					break;
				default:
			...
			$this->getResponse()->setHttpStatus(400);
			return $this->renderJSON(array('error' => Context::getI18n()->__('No valid field specified (%field%)', array('%field%' => $request->getParameter('field')))));
		}
		 ...
		 * @param Request $request
		 */
		public function runIssueRevertField(Request $request)
		{
			if ($issue_id = $request->getParameter('issue_id'))
			{
				try
			...
			switch ($request->getParameter('field'))
			{
				...
				case 'issuetype':
					...
					$field = ($issue->getIssuetype() instanceof \TBGIssuetype) ? array('id' => $issue->getIssuetype()->getID(), 'name' => $issue->getIssuetype()->getName(), 'src' => htmlspecialchars(Context::getTBGPath() . 'themes/' . \TBGSettings::getThemeName() . '/' . $issue->getIssuetype()->getIcon() . '_small.png')) : array('id' => 0);
					$visible_fields = ($issue->getIssuetype() instanceof \TBGIssuetype) ? $issue->getProject()->getVisibleFieldsArray($issue->getIssuetype()->getID()) : array();
		 ...
		 * @param Request $request
		 */
		public function runReopenIssue(Request $request)
		{
			if ($issue_id = $request->getParameter('issue_id'))
			{
				try
				...
				{
					return $this->return404(Context::getI18n()->__('This issue does not exist'));
				}
			}
			else
			{
		 ...
		 * @param Request $request
		 */
		public function runCloseIssue(Request $request)
		{
			if ($issue_id = $request->getParameter('issue_id'))
			{
				try
		...
		public function runMarkAsDuplicate(Request $request)
		{
			...
			try
			{
				$issue2 = Context::factory()->TBGIssue($request->getParameter('duplicate_issue'));
			}
			catch (Exception $e)
			{
				return $this->return404(Context::getI18n()->__('The issue to be set as the duplicate does not exist'));
			}
			if ($request->hasParameter('set_status'))
			{
				$issue->setStatus($request->getParameter('status_id'));
		...
		{
			$this->forward403unless($request->isMethod(Request::POST));
			$this->users = array();
		...
		public function runGetUploadStatus(Request $request)
		{
			...
			{
				$file = Context::factory()->TBGFile($status['file_id']);
				...
				$issue = Context::factory()->TBGIssue($request->getParameter('issue_id'));
		...
		public function runUpload(Request $request)
		{
			...
			if ($canupload)
			{
				try
				{
					$file = $request->handleUpload('uploader_file');
					if ($file instanceof TBGFile)
					{
						switch ($request->getParameter('mode'))
						{
							case 'issue':
								...
								{
									$comment->setContent(Context::getI18n()->__('A file was uploaded. %link_to_file% This comment was attached: %comment%', array('%comment%' => "\n\n".$request->getRawParameter('comment'), '%link_to_file%' => "[[File:{$file->getOriginalFilename()}|thumb|{$request->getParameter('uploader_file_description')}]]")));
								}
								else
								{
									$comment->setContent(Context::getI18n()->__('A file was uploaded. %link_to_file%', array('%link_to_file%' => "[[File:{$file->getOriginalFilename()}|thumb|{$request->getParameter('uploader_file_description')}]]")));
								}
								$comment->save();
								break;
							case 'article':
					...
					$this->error = Context::getI18n()->__('An unhandled error occured with the upload');
				}
				catch (Exception $e)
				{
					$this->getResponse()->setHttpStatus(400);
			...
			{
//				$this->getResponse()->setHttpStatus(401);
				$this->error = Context::getI18n()->__('You are not allowed to attach files here');
			}
			if (!$apc_exists)
			{
				switch ($request->getParameter('mode'))
				{
					...
					case 'article':
						if (!$article instanceof TBGWikiArticle) break;
						$this->forward(Context::getRouting()->generate('publish_article_attachments', array('article_name' => $article->getName())));
						break;
				}
			}
		...
		public function runDetachFile(Request $request)
		{
			try
			{
				switch ($request->getParameter('mode'))
				{
					...
					case 'article':
						...
						{
							$article->removeFile(Context::factory()->TBGFile((int) $request->getParameter('file_id')));
			...
			return $this->renderJSON(array('failed' => true, 'error' => Context::getI18n()->__('Invalid mode')));
		}
		...
		public function runGetFile(Request $request)
		{
			...
			$this->return404(Context::getI18n()->__('This file does not exist'));
		}
		...
		public function runAttachLinkToIssue(Request $request)
		{
			...
			if ($issue instanceof \TBGIssue && $issue->canAttachLinks())
			{
				if ($request->getParameter('link_url') != '')
				{
					...
					return $this->renderJSON(array('failed' => false, 'message' => Context::getI18n()->__('Link attached!'), 'attachmentcount' => (count($issue->getFiles()) + count($issue->getLinks())), 'content' => $this->getTemplateHTML('main/attachedlink', array('issue' => $issue, 'link_id' => $link_id, 'link' => array('description' => $request->getParameter('description'), 'url' => $request->getParameter('link_url'))))));
				}
				return $this->renderJSON(array('failed' => true, 'error' => Context::getI18n()->__('You have to provide a link URL, otherwise we have nowhere to link to!')));
			}
			return $this->renderJSON(array('failed' => true, 'error' => Context::getI18n()->__('You can not attach links to this issue')));
		}
		...
		public function runRemoveLinkFromIssue(Request $request)
		{
			...
			if ($issue instanceof \TBGIssue && $issue->canRemoveAttachments())
			{
				if ($request->getParameter('link_id') != 0)
				{
					...
					return $this->renderJSON(array('failed' => false, 'attachmentcount' => (count($issue->getFiles()) + count($issue->getLinks())), 'message' => Context::getI18n()->__('Link removed!')));
				}
				return $this->renderJSON(array('failed' => true, 'error' => Context::getI18n()->__('You have to provide a valid link id')));
			}
			return $this->renderJSON(array('failed' => true, 'error' => Context::getI18n()->__('You can not remove items from this issue')));
		}
		...
		public function runRemoveLink(Request $request)
		{
			if ($request->getParameter('link_id') != 0)
			{
				...
				return $this->renderJSON(array('failed' => false, 'message' => Context::getI18n()->__('Link removed!')));
			}
			return $this->renderJSON(array('failed' => true, 'error' => Context::getI18n()->__('You have to provide a valid link id')));
		}
		...
		{
			$comment = Context::factory()->TBGComment($request->getParameter('comment_id'));
			...
			{							
				...
				{
					unset($comment);
					$comment = Context::factory()->TBGComment((int) $request->getParameter('comment_id'));
					$comment->delete();
			...
			{
				return $this->renderJSON(array('failed' => true, 'error' => Context::getI18n()->__('Comment ID is invalid')));
			}
		}
		...
		public function runUpdateComment(Request $request)
		{
			...
			{							
				...
				else
				{
					...
					{
						return $this->renderJSON(array('failed' => true, 'error' => Context::getI18n()->__('The comment must have some content')));
					}
					...
					{
						$comment->setTitle(Context::getI18n()->__('Untitled comment'));
					}
					else
					{
		...
		public function listenViewIssuePostError(TBGEvent $event)
		{
			if (Context::hasMessage('comment_error'))
			{
				...
				$this->error = Context::getMessageAndClear('comment_error');
				$this->comment_error_title = Context::getMessageAndClear('comment_error_title');
				$this->comment_error_body = Context::getMessageAndClear('comment_error_body');
			}
		}
		
		public function runAddComment(Request $request)
		{
			$i18n = Context::getI18n();
			$comment = null;
			...
			try
			{
				...
				else
				{
					...
					if ($comment_applies_type == TBGComment::TYPE_ISSUE && !$request->isAjaxCall())
					{
						...
						$issue = Context::factory()->TBGIssue($request->getParameter('comment_applies_id'));
						$issue->save(false);
					}
					...
					switch ($comment_applies_type)
					{
						case TBGComment::TYPE_ISSUE:
							$comment_html = $this->getTemplateHTML('main/comment', array('comment' => $comment, 'issue' => Context::factory()->TBGIssue($request->getParameter('comment_applies_id'))));
							break;
			...
			catch (Exception $e)
			{
				...
				{
					Context::setMessage('comment_error', $e->getMessage());
					Context::setMessage('comment_error_body', $request->getParameter('comment_body'));
					Context::setMessage('comment_error_title', $request->getParameter('comment_title'));
					Context::setMessage('comment_error_visibility', $request->getParameter('comment_visibility'));
				}
			}
			if ($request->isAjaxCall())
		...
		public function runGetBackdropPartial(Request $request)
		{
			try
			{
				...
				switch ($request->getParameter('key'))
				{
					case 'usercard':
						...
						{
							$user = Context::factory()->TBGUser($user_id);
							$options['user'] = $user;
						}
						break;
					case 'login':
					...
					case 'workflow_transition':
						$transition = Context::factory()->TBGWorkflowTransition($request->getParameter('transition_id'));
					...
					case 'milestone':
						...
						$options['project'] = Context::factory()->TBGProject($request->getParameter('project_id'));
						if ($request->hasParameter('milestone_id'))
							$options['milestone'] = Context::factory()->TBGMilestone($request->getParameter('milestone_id'));
						break;
					case 'project_build':
						$template_name = 'configuration/build';
						$options['project'] = Context::factory()->TBGProject($request->getParameter('project_id'));
						if ($request->hasParameter('build_id'))
							$options['build'] = Context::factory()->TBGBuild($request->getParameter('build_id'));
						break;
					case 'project_icons':
					...
					case 'project_config':
						...
						$project = Context::factory()->TBGProject($request->getParameter('project_id'));
						$options['project'] = $project;
						$options['section'] = $request->getParameter('section', 'info');
						if ($request->hasParameter('edition_id'))
						{
							$edition = Context::factory()->TBGEdition($request->getParameter('edition_id'));
					...
					case 'issue_add_item':
						...
						break;
					case 'client_users':
						$options['client'] = Context::factory()->TBGClient($request->getParameter('client_id'));
			...
			{
				$this->getResponse()->setHttpStatus(400);
				return $this->renderJSON(array('failed' => true, 'error' => Context::getI18n()->__('An error occured: %error_message%', array('%error_message%' => $e->getMessage()))));
			}
			$this->getResponse()->setHttpStatus(400);
			return $this->renderJSON(array('failed' => true, 'error' => Context::getI18n()->__('Invalid template or parameter')));
		}

		public function runFindIssue(Request $request)
		{
			$status = 200;
			$message = null;
			if ($issue_id = $request->getParameter('issue_id'))
			{
				...
				{
					$status = 400;
					$message = Context::getI18n()->__('Could not find this issue');
				}
			}
			elseif ($request->hasParameter('issue_id'))
			{
				$status = 400;
				$message = Context::getI18n()->__('Please provide an issue number');
			}
			...
			{
//				$status = 400;
//				$message = Context::getI18n()->__('Please enter something to search for (3 characters or more) %searchfor%', array('searchfor' => $searchfor));
				$issues = array();
				$count = 0;
			}
			else
		...
		public function runFindDuplicateIssue(Request $request)
		{
			$status = 200;
			$message = null;
			if ($issue_id = $request->getParameter('issue_id'))
		...
		public function runRelateIssues(Request $request)
		{
			...
			$cc = 0;
			$message = Context::getI18n()->__('Unknown error');
			if (count($related_issues))
			{
				...
				foreach ($related_issues as $issue_id)
				{
					try
					{
						$related_issue = Context::factory()->TBGIssue((int) $issue_id);
					...
					{
						$this->getResponse()->setHttpStatus(400);
						return $this->renderJSON(array('failed' => true, 'error' => Context::getI18n()->__('An error occured when relating issues: %error%', array('%error%' => $e->getMessage()))));
					}
				}
			}
			else
			{
				$message = Context::getI18n()->__('Please select at least one issue');
			}

			if ($cc > 0)
			{
				return $this->renderJSON(array('failed' => false, 'content' => $content, 'message' => Context::getI18n()->__('The related issue was added')));
			}
			else
			{
				return $this->renderJSON(array('failed' => true, 'error' => Context::getI18n()->__('An error occured when relating issues: %error%', array('%error%' => $message))));
			}
		}
		...
		public function runToggleFriend(Request $request)
		{
			try
			{
				$friend_user = Context::factory()->TBGUser($request->getParameter('user_id'));
			...
			{
				return $this->renderJSON(array('failed' => true, 'error' => Context::getI18n()->__('Could not add or remove friend')));
			}
		}
		...
		public function runToggleAffectedConfirmed(Request $request)
		{
			...
			try
			{
				...
				switch ($request->getParameter('affected_type'))
				{
					case 'edition':
						...
						{
							$this->getResponse()->setHttpStatus(400);
							return $this->renderJSON(array('failed' => true, 'error' => Context::getI18n()->__('Editions are disabled')));
						}
						...
						if ($edition['confirmed'] == true)
						{
							...
							$message = Context::getI18n()->__('Edition <b>%edition%</b> is now unconfirmed for this issue', array('%edition%' => $edition['edition']->getName()));
							$alt = Context::getI18n()->__('No');
						...
						else
						{
							...
							$message = Context::getI18n()->__('Edition <b>%edition%</b> is now confirmed for this issue', array('%edition%' => $edition['edition']->getName()));
							$alt = Context::getI18n()->__('Yes');
					...
					case 'component':
						...
						{
							$this->getResponse()->setHttpStatus(400);
							return $this->renderJSON(array('failed' => true, 'error' => Context::getI18n()->__('Components are disabled')));
						}
						...
						if ($component['confirmed'] == true)
						{
							...
							$message = Context::getI18n()->__('Component <b>%component%</b> is now unconfirmed for this issue', array('%component%' => $component['component']->getName()));
							$alt = Context::getI18n()->__('No');
							$src = image_url('action_cancel_small.png');
						}
						else
						{
							...
							$message = Context::getI18n()->__('Component <b>%component%</b> is now confirmed for this issue', array('%component%' => $component['component']->getName()));
							$alt = Context::getI18n()->__('Yes');
							$src = image_url('action_ok_small.png');
						}
					...
					case 'build':
						...
						{
							$this->getResponse()->setHttpStatus(400);
							return $this->renderJSON(array('failed' => true, 'error' => Context::getI18n()->__('Releases are disabled')));
						}
						...
						if ($build['confirmed'] == true)
						{
							...
							$message = Context::getI18n()->__('Release <b>%build%</b> is now unconfirmed for this issue', array('%build%' => $build['build']->getName()));
							$alt = Context::getI18n()->__('No');
							$src = image_url('action_cancel_small.png');
						}
						else
						{
							...
							$message = Context::getI18n()->__('Release <b>%build%</b> is now confirmed for this issue', array('%build%' => $build['build']->getName()));
							$alt = Context::getI18n()->__('Yes');
							$src = image_url('action_ok_small.png');
						}
			...
			{
				$this->getResponse()->setHttpStatus(400);
				return $this->renderJSON(array('failed' => true, 'error' => Context::getI18n()->__('An internal error has occured')));
			}
		}
			...
			try
			{
				...
				switch ($request->getParameter('affected_type'))
				{
					case 'edition':
						...
						$message = Context::getI18n()->__('Edition <b>%edition%</b> is no longer affected by this issue', array('%edition%' => $edition['edition']->getName()));
					...
					case 'component':
						...
						$message = Context::getI18n()->__('Component <b>%component%</b> is no longer affected by this issue', array('%component%' => $component['component']->getName()));
					...
					case 'build':
						...
						$message = Context::getI18n()->__('Release <b>%build%</b> is no longer affected by this issue', array('%build%' => $build['build']->getName()));
		...
		public function runStatusAffected(Request $request)
		{
			...
			{
				$issue = Context::factory()->TBGIssue($request->getParameter('issue_id'));
				$status = Context::factory()->TBGStatus($request->getParameter('status_id'));
				if (!$issue->canEditIssue())
				{
					$this->getResponse()->setHttpStatus(400);
					return $this->renderJSON(array('failed' => true, 'error' => Context::getI18n()->__('You are not allowed to do this')));
				...
				switch ($request->getParameter('affected_type'))
				{
					case 'edition':
						...
						$message = Context::getI18n()->__('Edition <b>%edition%</b> is now %status%', array('%edition%' => $edition['edition']->getName(), '%status%' => $status->getName()));
					...
					case 'component':
						...
						$message = Context::getI18n()->__('Component <b>%component%</b> is now %status%', array('%component%' => $component['component']->getName(), '%status%' => $status->getName()));
					...
					case 'build':
						...
						$message = Context::getI18n()->__('Release <b>%build%</b> is now %status%', array('%build%' => $build['build']->getName(), '%status%' => $status->getName()));
			...
			try
			{
				...
				switch ($request->getParameter('item_type'))
				{
					case 'edition':
						...
						$edition = Context::factory()->TBGEdition($request->getParameter('which_item_edition'));
						
						if (\TBGIssueAffectsEditionTable::getTable()->getByIssueIDandEditionID($issue->getID(), $edition->getID()))
						{
							$this->getResponse()->setHttpStatus(400);
							return $this->renderJSON(array('failed' => true, 'error' => Context::getI18n()->__('%item% is already affected by this issue', array('%item%' => $edition->getName()))));
						}
						...
						$item = $edition;
						$itemtype = 'edition';
						$itemtypename = Context::getI18n()->__('Edition');
						...
						$message = Context::getI18n()->__('Edition <b>%edition%</b> is now affected by this issue', array('%edition%' => $edition['edition']->getName()));
					...
					case 'component':
						...
						$component = Context::factory()->TBGComponent($request->getParameter('which_item_component'));
						
						if (\TBGIssueAffectsComponentTable::getTable()->getByIssueIDandComponentID($issue->getID(), $component->getID()))
						{
							$this->getResponse()->setHttpStatus(400);
							return $this->renderJSON(array('failed' => true, 'error' => Context::getI18n()->__('%item% is already affected by this issue', array('%item%' => $component->getName()))));
						}
						...
						$item = $component;
						$itemtype = 'component';
						$itemtypename = Context::getI18n()->__('Component');
						$content = get_template_html('main/affecteditem', array('item' => $item, 'itemtype' => $itemtype, 'itemtypename' => $itemtypename, 'issue' => $issue, 'statuses' => $statuses));
						
						$message = Context::getI18n()->__('Component <b>%component%</b> is now affected by this issue', array('%component%' => $component['component']->getName()));
					...
					case 'build':
						...
						$build = Context::factory()->TBGBuild($request->getParameter('which_item_build'));

						
						if (\TBGIssueAffectsBuildTable::getTable()->getByIssueIDandBuildID($issue->getID(), $build->getID()))
						{
							$this->getResponse()->setHttpStatus(400);
							return $this->renderJSON(array('failed' => true, 'error' => Context::getI18n()->__('%item% is already affected by this issue', array('%item%' => $build->getName()))));
						}
						...
						$item = $build;
						$itemtype = 'build';
						$itemtypename = Context::getI18n()->__('Release');
						$content = get_template_html('main/affecteditem', array('item' => $item, 'itemtype' => $itemtype, 'itemtypename' => $itemtypename, 'issue' => $issue, 'statuses' => $statuses));
												
						$message = Context::getI18n()->__('Release <b>%build%</b> is now affected by this issue', array('%build%' => $build['build']->getName()));
		 ...
		 * 
		 */
		public function runReset(Request $request)
		{			
			$i18n = Context::getI18n();
			...
			if ($request->hasParameter('user') && $request->hasParameter('reset_hash'))
			{
				...
				{
					Context::setMessage('login_message_err', $i18n->__($e->getMessage()));
				}
			}
			else
			{
				Context::setMessage('login_message_err', $i18n->__('An internal error has occured'));
			}
			return $this->forward(Context::getRouting()->generate('login_page'));
		}
		...
		public function runIssueGetTempFieldValue(Request $request)
		{
			switch ($request->getParameter('field'))
			{
				case 'assigned_to':
					...
					{
						$identifiable = Context::factory()->TBGUser($request->getParameter('value'));
					...
					{
						$identifiable = Context::factory()->TBGTeam($request->getParameter('value'));
