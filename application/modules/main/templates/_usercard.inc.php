<div class="backdrop_box large backdrop_detail_content" id="user_details_popup">
	<div id="backdrop_detail_content" class="rounded_top" style="padding: 10px; text-align: left;">
		...
		<?php if (TBGUser::isThisGuest() == false): ?>
			...
			<?php if ($user->getID() != \caspar\core\Caspar::getUser()->getID() && !(\caspar\core\Caspar::getUser()->isFriend($user)) && !$user->isGuest()): ?>
				<div id="friends_link_<?php echo $user->getUsername() . '_' . $rnd_no; ?>">
					<span style="padding: 2px; <?php if (\caspar\core\Caspar::getUser()->isFriend($user)): ?> display: none;<?php endif; ?>" id="add_friend_<?php echo $user->getID() . '_' . $rnd_no; ?>">
					...
					<span style="padding: 2px; <?php if (!\caspar\core\Caspar::getUser()->isFriend($user)): ?> display: none;<?php endif; ?>" id="remove_friend_<?php echo $user->getID() . '_' . $rnd_no; ?>">
