<div class="rounded_box white<?php if (isset($absolute) && $absolute): ?> shadowed<?php endif; ?><?php if (isset($classes)): echo ' '.$classes; endif; ?>" id="<?php echo $html_id; ?>" style="<?php if (isset($absolute) && $absolute): ?>position: absolute;<?php else: ?>margin: 5px 0 5px 0; clear: both;<?php endif; ?> display: none; width: 324px; z-index: 10001; <?php if (isset($style)): foreach ($style as $key => $val): echo ' ' . $key . ': ' . $val . ';'; endforeach; endif; ?>">
	<form id="<?php echo $base_id; ?>_form" accept-charset="<?php echo \caspar\core\Caspar::getI18n()->getCharset(); ?>" method="post" action="" onsubmit="TBG.Main.findIdentifiable('<?php echo make_url('main_find_identifiable'); ?>', '<?php echo $base_id; ?>');return false;">
	...
	<div class="dropdown_content">
		...
		<a href="javascript:void(0);" onclick="<?php echo str_replace(array(urlencode('%identifiable_type%'), '%identifiable_type%', urlencode('%identifiable_value%'), '%identifiable_value%'), array(TBGIdentifiableClass::TYPE_USER, TBGIdentifiableClass::TYPE_USER, $csp_user->getID(), $csp_user->getID()), $callback); ?>"><?php echo __('Select yourself'); ?> (<?php echo $csp_user->getUsername(); ?>)</a><br>
		<?php if (count($csp_user->getFriends()) == 0): ?>
			<span class="faded_out"><?php echo __("Your friends will appear here"); ?></span><br>
		<?php else: ?>
			<?php include_component('main/identifiableselectorresults', array('users' => $csp_user->getFriends(), 'callback' => $callback, 'team_callback' => ((isset($team_callback)) ? $team_callback : null))); ?>
		<?php endif; ?>
		<?php if (count($csp_user->getTeams()) > 0): ?>
		<br>
		<b><?php echo __('Or select one of your teams'); ?>:</b><br>
			<?php foreach ($csp_user->getTeams() as $team): ?>
