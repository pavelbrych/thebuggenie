...
<?php else: ?>
	...
	<?php endif; ?>
	<a href="javascript:void(0);" onclick="$('bud_<?php echo $user->getUsername() . "_" . $rnd_no; ?>').toggle();" style="font-weight: normal;"<?php if ($csp_user->isFriend($user)): ?> class="friend" title="<?php echo __('This is one of your friends'); ?>"<?php endif; ?>><?php echo $user->getBuddyname(); ?></a>
	<div id="bud_<?php echo $user->getUsername() . '_' . $rnd_no; ?>" style="z-index: 100; width: 300px; display: none; position: absolute;" class="rounded_box white shadowed user_popup dropdown_box">
		<div style="padding: 3px;">
			...
			<?php \caspar\core\Event::createNew('core', 'useractions_top', $user)->trigger(); ?>
			<?php if (TBGUser::isThisGuest() == false && $user->getID() != $csp_user->getID()): ?>
				<div style="padding: 2px;<?php if ($csp_user->isFriend($user)): ?> display: none;<?php endif; ?>" id="add_friend_<?php echo $user->getID() . '_' . $rnd_no; ?>">
				...
				<div style="padding: 2px;<?php if (!$csp_user->isFriend($user)): ?> display: none;<?php endif; ?>" id="remove_friend_<?php echo $user->getID() . '_' . $rnd_no; ?>">
			...
			<?php \caspar\core\Event::createNew('core', 'useractions_bottom', $user)->trigger(); ?>
		</div>
