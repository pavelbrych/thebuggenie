<?php
	 ...
	 * @subpackage tables
	 */
	class TBGIssueSpentTimes extends ScopedTable
	{
		...
		public function getSpentTimesByDateAndIssueIDs($startdate, $enddate, $issue_ids)
		{
			...
			if (count($issue_ids))
			{
				...
				$crit = $this->getCriteria();
				$crit->addWhere(self::EDITED_AT, $enddate, \b2db\Criteria::DB_LESS_THAN_EQUAL);
				$crit->addJoin(Caspar::getB2DBInstance()->getTable('TBGIssueRelationsTable'), TBGIssueRelationsTable::PARENT_ID, self::ISSUE_ID);
