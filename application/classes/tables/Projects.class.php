<?php
	...
	class Projects extends ScopedTable 
	{
		...
		protected function _setup()
		{
			...
			parent::_addForeignKeyColumn(self::WORKFLOW_SCHEME_ID, $this->_connection->getTable('\\thebuggenie\\tables\\WorkflowSchemes'), WorkflowSchemes::ID);
			parent::_addForeignKeyColumn(self::ISSUETYPE_SCHEME_ID, $this->_connection->getTable('\\thebuggenie\\tables\\IssuetypeSchemes'), IssuetypeSchemes::ID);
			parent::_addForeignKeyColumn(self::CLIENT, $this->_connection->getTable('\\thebuggenie\\tables\\Clients'), Clients::ID);
			parent::_addForeignKeyColumn(self::SMALL_ICON, $this->_connection->getTable('\\thebuggenie\\tables\\Files'), Files::ID);
			parent::_addForeignKeyColumn(self::LARGE_ICON, $this->_connection->getTable('\\thebuggenie\\tables\\Files'), Files::ID);
		}
		...
		public function getByUserID($user_id)
		{
			...
			$ctn->addWhere(self::LEAD_TYPE, \thebuggenie\core\IdentifiableClass::TYPE_USER);
			$ctn->addWhere(self::SCOPE, \thebuggenie\core\Context::getScope()->getID());
			$crit->addWhere($ctn);
			$ctn = $crit->returnCriterion(self::OWNER, $user_id);
			$ctn->addWhere(self::SCOPE, \thebuggenie\core\Context::getScope()->getID());
			$ctn->addWhere(self::OWNER_TYPE, \thebuggenie\core\IdentifiableClass::TYPE_USER);
			$crit->addOr($ctn);
			...
			if ($res = $this->doSelect($crit))
			{
				...
				{
					$return_array[$row->get(self::ID)] = \caspar\core\Caspar::factory()->manufacture('\\thebuggenie\\entities\\Project', $row->get(self::ID), $row);
				}
			}
			return $return_array;
		}
