<?php
	...
	class Log extends ScopedTable 
	{
		...
		{
			$crit = $this->getCriteria();
			$crit->addJoin($this->_connection->getTable('\\thebuggenie\\tables\\Issues'), Issues::ID, self::TARGET);
			$crit->addWhere(self::TARGET_TYPE, self::TYPE_ISSUE);
		...
		public function getLast15IssueCountsByProjectID($project_id)
		{
			...
			{
				$crit = $this->getCriteria();
				$joinedtable = $crit->addJoin($this->_connection->getTable('\\thebuggenie\\tables\\Issues'), Issues::ID, self::TARGET);
				$crit->addWhere(self::TARGET_TYPE, self::TYPE_ISSUE);
				$crit->addWhere(self::CHANGE_TYPE, array(self::LOG_ISSUE_CREATED, self::LOG_ISSUE_CLOSE), Criteria::DB_IN);
				$crit->addWhere(Issues::PROJECT_ID, $project_id);
				$crit->addWhere(Issues::DELETED, false);
				$crit->addJoin($this->_connection->getTable('\\thebuggenie\\tables\\IssueTypes'), IssueTypes::ID, Issues::ISSUE_TYPE, array(), Criteria::DB_LEFT_JOIN, $joinedtable);
