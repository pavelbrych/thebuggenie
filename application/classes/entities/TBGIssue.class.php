<?php
	...
	class TBGIssue extends TBGChangeableItem
	{
		...
		public static function getIssueFromLink($issue_no, $project = null)
		{
			...
			if (is_numeric($issue_no))
			{
				try
				{
					...
					{
						$theIssue = \caspar\core\Caspar::factory()->manufacture('\thebuggenie\entities\Issue', $row->get(TBGIssuesTable::ID), $row);
					}
				}
				catch (Exception $e)
				{
			...
			else
			{
				...
				\caspar\core\Logging::log('exploding done');
			}
		...
		{
			\caspar\core\Logging::log('checking access to issue ' . $this->getFormattedIssueNo());
			$i_id = $this->getID();
			$specific_access = \caspar\core\Caspar::getUser()->hasPermission("canviewissue", $i_id, 'core', true, null);
			if ($specific_access !== null)
			{
				\caspar\core\Logging::log('done checking, returning specific access ' . (($specific_access) ? 'allowed' : 'denied'));
				return $specific_access;
			}
			if ($this->getPostedByID() == \caspar\core\Caspar::getUser()->getID())
			{
				\caspar\core\Logging::log('done checking, allowed since this user posted it');
				return true;
			}
			if ($this->getOwnerType() == TBGIdentifiableClass::TYPE_USER && $this->getOwnerID() == \caspar\core\Caspar::getUser()->getID())
			{
				\caspar\core\Logging::log('done checking, allowed since this user owns it');
				return true;
			}
			if ($this->getAssigneeType() == TBGIdentifiableClass::TYPE_USER && $this->getAssigneeID() == \caspar\core\Caspar::getUser()->getID())
			{
				\caspar\core\Logging::log('done checking, allowed since this user is assigned to it');
				return true;
			}
			if (\caspar\core\Caspar::getUser()->hasPermission('canseegroupissues', 0, 'core', true, true) &&
				$this->getPostedByType() == TBGIdentifiableClass::TYPE_USER && 
				$this->getPostedBy()->getGroupID() == \caspar\core\Caspar::getUser()->getGroupID())
			{
				\caspar\core\Logging::log('done checking, allowed since this user is in same group as user that posted it');
				return true;
			}
			if (!\caspar\core\Caspar::getUser()->hasPermission('canseeallissues', 0, 'core', true, true))
			{
				\caspar\core\Logging::log('done checking, not allowed to access issues not posted by themselves');
				return false;
			}
			if ($this->getProject()->hasAccess())
			{
				\caspar\core\Logging::log('done checking, can access project');
				return true;
			}
			\caspar\core\Logging::log('done checking, denied');
			return false;
		}
		...
		public function getWorkflowStep()
		{
			...
			{
				$this->_workflow_step_id = \caspar\core\Caspar::factory()->manufacture('\thebuggenie\entities\WorkflowStep', $this->_workflow_step_id);
			}
		...
		protected function _populateAffected()
		{
			if ($this->_editions === null && $this->_builds === null && $this->_components === null)
			{
				...
				if ($res = Caspar::getB2DBInstance()->getTable('TBGIssueAffectsEditionTable')->getByIssueID($this->getID()))
				{
					while ($row = $res->getNextRow())
					{
						try
						{
							$this->_editions[$row->get(TBGIssueAffectsEditionTable::ID)] = array(	'edition' => \caspar\core\Caspar::factory()->manufacture('\thebuggenie\entities\Edition', $row->get(TBGIssueAffectsEditionTable::EDITION)),
														'status' => \caspar\core\Caspar::factory()->manufacture('\thebuggenie\entities\Status', $row->get(TBGIssueAffectsEditionTable::STATUS), $row),
				...
				if ($res = Caspar::getB2DBInstance()->getTable('TBGIssueAffectsBuildTable')->getByIssueID($this->getID()))
				{
					while ($row = $res->getNextRow())
					{
						try
						{
							$this->_builds[$row->get(TBGIssueAffectsBuildTable::ID)] = array(	'build' => \caspar\core\Caspar::factory()->manufacture('\thebuggenie\entities\Build', $row->get(TBGIssueAffectsBuildTable::BUILD)),
														'status' => \caspar\core\Caspar::factory()->manufacture('\thebuggenie\entities\Status', $row->get(TBGIssueAffectsBuildTable::STATUS), $row),
				...
				if ($res = Caspar::getB2DBInstance()->getTable('TBGIssueAffectsComponentTable')->getByIssueID($this->getID()))
				{
					while ($row = $res->getNextRow())
					{
						try
						{
							$this->_components[$row->get(TBGIssueAffectsComponentTable::ID)] = array(	'component' => \caspar\core\Caspar::factory()->manufacture('\thebuggenie\entities\Component', $row->get(TBGIssueAffectsComponentTable::COMPONENT)),
															'status' => \caspar\core\Caspar::factory()->manufacture('\thebuggenie\entities\Status', $row->get(TBGIssueAffectsComponentTable::STATUS), $row),
		...
		public function getDuplicateOf()
		{
			/*if (is_numeric($this->_duplicate_of))
			{
				try
				{
					$this->_duplicate_of = \caspar\core\Caspar::factory()->manufacture('\thebuggenie\entities\Issue', $this->_duplicate_of);
				}
				catch (Exception $e) 
				{
		...
		{
			$retval = null;
			if (\caspar\core\Caspar::getUser()->isGuest()) return false;
		...
		public function canEditIssueDetails()
		{
			...
			if (!$this->getProject()->canChangeIssuesWithoutWorkingOnThem())
			{
				...
				if ($this->getUserWorkingOnIssue()->getID() == \caspar\core\Caspar::getUser()->getID())
					return true;
		...
		public function getIssueType()
		{
			/*if (is_numeric($this->_issuetype))
			{
				try
				{
					$this->_issuetype = \caspar\core\Caspar::factory()->manufacture('\thebuggenie\entities\Issuetype', $this->_issuetype);
				}
				catch (Exception $e) 
				{
		...
		{
			$link_id = Caspar::getB2DBInstance()->getTable('TBGLinksTable')->addLinkToIssue($this->getID(), $url, $description);
			return $link_id;
		}
		...
		protected function _populateRelatedIssues()
		{
			if ($this->_parent_issues === null || $this->_child_issues === null)
			{
				...
				if ($res = Caspar::getB2DBInstance()->getTable('TBGIssueRelationsTable')->getRelatedIssues($this->getID()))
				{
					while ($row = $res->getNextRow())
					{
						try
						{
							...
							{
								$issue = \caspar\core\Caspar::factory()->manufacture('\thebuggenie\entities\Issue', $row->get(TBGIssueRelationsTable::CHILD_ID));
								$this->_child_issues[$row->get(TBGIssueRelationsTable::ID)] = $issue;
							}
							else
							{
								$issue = \caspar\core\Caspar::factory()->manufacture('\thebuggenie\entities\Issue', $row->get(TBGIssueRelationsTable::PARENT_ID));
		...
		public function getTasks()
		{
			if ($this->getProject()->isTasksEnabled())
			{
				...
				{
					$this->_tasks = array();
					if ($res = Caspar::getB2DBInstance()->getTable('TBGIssueTasksTable')->getByIssueID($this->getID()))
					{
		...
		public function getTags()
		{
			...
			{
				$this->_tags = array();
				if ($res = Caspar::getB2DBInstance()->getTable('TBGIssueTagsTable')->getByIssueID($this->getID()))
				{
					while ($row = $resultset->getNextRow())
					{
		...
		public function getReproducability()
		{
			if (is_numeric($this->_reproducability))
			{
				try
				{
					$this->_reproducability = \caspar\core\Caspar::factory()->manufacture('\thebuggenie\entities\Reproducability', $this->_reproducability);
				}
				catch (Exception $e)
				{
		...
		public function getPriority()
		{
			if (is_numeric($this->_priority))
			{
				try
				{
					$this->_priority = \caspar\core\Caspar::factory()->manufacture('\thebuggenie\entities\Priority', $this->_priority);
				}
				catch (Exception $e)
				{
					$this->_priority = null;
		...
		public function getMilestone()
		{
			/*if (is_numeric($this->_milestone))
			{
				try
				{
					$this->_milestone = \caspar\core\Caspar::factory()->manufacture('\thebuggenie\entities\Milestone', $this->_milestone);
				}
				catch (Exception $e)
				{
		...
		public function removeDependantIssue($issue_id)
		{
			...
			{
				$related_issue = \caspar\core\Caspar::factory()->manufacture('\thebuggenie\entities\Issue', $issue_id);
				if ($row->get(TBGIssueRelationsTable::PARENT_ID) == $this->getID())
				{
		...
		{
			if (!$row = Caspar::getB2DBInstance()->getTable('TBGIssueRelationsTable')->getIssueRelation($this->getID(), $related_issue->getID()))
			{
		...
		public function getAssignee()
		{
			if (is_numeric($this->_assigned_to))
			{
				try
				{
					...
					{
						$this->_assigned_to = \caspar\core\Caspar::factory()->manufacture('\\thebuggenie\\core\\User', $this->_assigned_to);
					}
					elseif ($this->_assigned_type == TBGIdentifiableClass::TYPE_TEAM)
					{
						$this->_assigned_to = \caspar\core\Caspar::factory()->manufacture('\\thebuggenie\\core\\Team', $this->_assigned_to);
					}
				}
				catch (Exception $e)
				{
		...
		public function getOwner()
		{
			if (is_numeric($this->_owner))
			{
				try
				{
					...
					{
						$this->_owner = \caspar\core\Caspar::factory()->manufacture('\\thebuggenie\\core\\User', $this->_owner);
					}
					elseif ($this->_owner_type == TBGIdentifiableClass::TYPE_TEAM)
					{
						$this->_owner = \caspar\core\Caspar::factory()->manufacture('\\thebuggenie\\core\\Team', $this->_owner);
					}
				}
				catch (Exception $e)
				{
		...
		public function getPostedBy()
		{
			if (is_numeric($this->_posted_by))
			{
				try
				{
					$this->_posted_by = \caspar\core\Caspar::factory()->manufacture('\\thebuggenie\\core\\User', $this->_posted_by);
				}
				catch (Exception $e)
				{
		...
		{
			$uid = ($system) ? 0 : \caspar\core\Caspar::getUser()->getID();
		...
		protected function _populateLinks()
		{
			if ($this->_links === null)
			{
				$this->_links = Caspar::getB2DBInstance()->getTable('TBGLinksTable')->getByIssueID($this->getID());
			}
		}
		...
		{
			if ($res = Caspar::getB2DBInstance()->getTable('TBGLinksTable')->removeByIssueIDandLinkID($this->getID(), $link_id))
			{
		...
		public function whenReopened()
		{
			...
			$crit->addOrderBy(TBGLogTable::TIME, 'desc');
			$row = TBGLogTable::getTable()->doSelectOne($crit);
			
			return ($row) ? $row->get(TBGLogTable::TIME) : null;
		}
				...
				switch ($customdatatype->getType())
				{
					...
					case TBGCustomDatatype::STATUS_CHOICE:
						...
						try
						{
							...
							{
								case TBGCustomDatatype::EDITIONS_CHOICE:
									$option_object = \caspar\core\Caspar::factory()->manufacture('\thebuggenie\entities\Edition', $this->getCustomField($key));
									break;
								case TBGCustomDatatype::COMPONENTS_CHOICE:
									$option_object = \caspar\core\Caspar::factory()->manufacture('\thebuggenie\entities\Component', $this->getCustomField($key));
									break;
								case TBGCustomDatatype::RELEASES_CHOICE:
									$option_object = \caspar\core\Caspar::factory()->manufacture('\thebuggenie\entities\Build', $this->getCustomField($key));
									break;
								case TBGCustomDatatype::STATUS_CHOICE:
									$option_object = \caspar\core\Caspar::factory()->manufacture('\thebuggenie\entities\Status', $this->getCustomField($key));
									break;
							}
						}
						catch (Exception $e) {}
		...
		public function _preSave($is_new)
		{
			if ($is_new)
			{
				...
				if (!$this->_posted_by) $this->_posted_by = \caspar\core\Caspar::getUser();
								...
								switch ($customdatatype->getType())
								{
									...
									case TBGCustomDatatype::STATUS_CHOICE:
										...
										try
										{
											...
											{
												case TBGCustomDatatype::EDITIONS_CHOICE:
													$old_object = \caspar\core\Caspar::factory()->manufacture('\thebuggenie\entities\Edition', $value['original_value']);
													break;
												case TBGCustomDatatype::COMPONENTS_CHOICE:
													$old_object = \caspar\core\Caspar::factory()->manufacture('\thebuggenie\entities\Component', $value['original_value']);
													break;
												case TBGCustomDatatype::RELEASES_CHOICE:
													$old_object = \caspar\core\Caspar::factory()->manufacture('\thebuggenie\entities\Build', $value['original_value']);
												case TBGCustomDatatype::STATUS_CHOICE:
													$old_object = \caspar\core\Caspar::factory()->manufacture('\thebuggenie\entities\Status', $value['original_value']);
													break;
											}
										}
										catch (Exception $e) {}
										try
										{
											...
											{
												case TBGCustomDatatype::EDITIONS_CHOICE:
													$new_object = \caspar\core\Caspar::factory()->manufacture('\thebuggenie\entities\Edition', $this->getCustomField($key));
													break;
												case TBGCustomDatatype::COMPONENTS_CHOICE:
													$new_object = \caspar\core\Caspar::factory()->manufacture('\thebuggenie\entities\Component', $this->getCustomField($key));
													break;
												case TBGCustomDatatype::RELEASES_CHOICE:
													$new_object = \caspar\core\Caspar::factory()->manufacture('\thebuggenie\entities\Build', $this->getCustomField($key));
												case TBGCustomDatatype::STATUS_CHOICE:
													$new_object = \caspar\core\Caspar::factory()->manufacture('\thebuggenie\entities\Status', $this->getCustomField($key));
													break;
											}
										}
										catch (Exception $e) {}
			...
			if ($is_saved_estimated)
			{
				Caspar::getB2DBInstance()->getTable('TBGIssueEstimates')->saveEstimate($this->getID(), $this->_estimated_months, $this->_estimated_weeks, $this->_estimated_days, $this->_estimated_hours, $this->_estimated_points);
			}

			if ($is_saved_spent)
			{
				Caspar::getB2DBInstance()->getTable('TBGIssueSpentTimes')->saveSpentTime($this->getID(), $this->_spent_months, $this->_spent_weeks, $this->_spent_days, $this->_spent_hours, $this->_spent_points);
			}
		...
		protected function _postSave($is_new)
		{
			...
			if (isset($this->related_issues_to_save))
			{
				...
				{
					$related_issue = \caspar\core\Caspar::factory()->manufacture('\thebuggenie\entities\Issue', $i_id);
					$related_issue->save();
				}
			}
