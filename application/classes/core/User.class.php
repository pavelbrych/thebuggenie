<?php
	 ...
	 * @subpackage core
	 */
	class User extends IdentifiableClass 
	{
		
		static protected $_b2dbtablename = '\\thebuggenie\\tables\Users';
		 ...
		 * The user state
		 *
		 * @var \thebuggenie\entities\Userstate
		 * @Class \thebuggenie\entities\Userstate
		 */
		...
		{
			if ($row = Caspar::getB2DBInstance()->getTable('\\thebuggenie\\tables\Users')->getByUsername($username, $scope))
			{
				return \caspar\core\Caspar::factory()->manufacture('\\thebuggenie\\core\\User', $row->get(Users::ID), $row);
			}
			return null;
		}
		...
		public static function getByOpenID($identity)
		{
			...
			{
				$user = \caspar\core\Caspar::factory()->manufacture('\\thebuggenie\\core\\User', $user_id);
			}
			elseif (!Settings::isUsingExternalAuthenticationBackend())
			{
				$user = new TBGUser();
		...
		public static function getAll()
		{
			...
			{
				self::$_users = array();
				if ($res = Caspar::getB2DBInstance()->getTable('TBGUsersTable')->getAll())
				{
					while ($row = $res->getNextRow())
					{
						self::$_users[$row->get(Users::ID)] = \caspar\core\Caspar::factory()->manufacture('\\thebuggenie\\core\\User', $row->get(Users::ID), $row);
					}
				}
			}
			return self::$_users;
		 ...
		 * @param \thebuggenie\entities\Group $admin_group
		 * @param \thebuggenie\entities\Group $user_group
		 * @param \thebuggenie\entities\Group $guest_group 
		 */
		public static function loadFixtures(TBGScope $scope, \thebuggenie\entities\Group $admin_group, \thebuggenie\entities\Group $user_group, \thebuggenie\entities\Group $guest_group)
		{
			...
			Settings::saveSetting('defaultuserid', $guestuser->getID(), 'core', $scope->getID());
		}
		...
		{
			$salt = ($salt !== null) ? $salt : Settings::getPasswordSalt();
		...
		public static function loginCheck($username = null, $password = null)
		{
			try
			{
				...
				{
					if (Caspar::getRequest()->hasCookie('tbg3_username') && Caspar::getRequest()->hasCookie('tbg3_password'))
					{
						$username = Caspar::getRequest()->getCookie('tbg3_username');
						$password = Caspar::getRequest()->getCookie('tbg3_password');
						$row = Caspar::getB2DBInstance()->getTable('\\thebuggenie\\tables\Users')->getByUsernameAndPassword($username, $password);
						$raw = false;
				...
				if (Settings::isUsingExternalAuthenticationBackend() && $username !== null && $password !== null)
				{
					$external = true;
					\caspar\core\Logging::log('Authenticating with backend: '.Settings::getAuthenticationBackend(), 'auth', \caspar\core\Logging::LEVEL_INFO);
					try
					{
						$mod = Caspar::getModule(Settings::getAuthenticationBackend());
						if ($mod->getType() !== TBGModule::MODULE_AUTH)
						{
							\caspar\core\Logging::log('Auth module is not the right type', 'auth', \caspar\core\Logging::LEVEL_FATAL);
						}
						if (Caspar::getRequest()->hasCookie('tbg3_username') && Caspar::getRequest()->hasCookie('tbg3_password'))
						{
				...
				{
					$external = true;
					\caspar\core\Logging::log('Authenticating without credentials with backend: '.Settings::getAuthenticationBackend(), 'auth', \caspar\core\Logging::LEVEL_INFO);
					try
					{
						$mod = Caspar::getModule(Settings::getAuthenticationBackend());
						if ($mod->getType() !== TBGModule::MODULE_AUTH)
				...
				{
					$external = false;
					\caspar\core\Logging::log('Using internal authentication', 'auth', \caspar\core\Logging::LEVEL_INFO);
					...
					if (!$row)
					{
						...
						$row = Caspar::getB2DBInstance()->getTable('\\thebuggenie\\tables\Users')->getByUsernameAndPassword($username, self::hashPassword($password));

						if(!$row)
						{
							// This is a legacy account from a 3.1 upgrade - try sha1 salted
							$salt = Settings::getPasswordSalt();
							$row = Caspar::getB2DBInstance()->getTable('\\thebuggenie\\tables\Users')->getByUsernameAndPassword($username, sha1($password.$salt));
							if(!$row)
							{
								// Invalid
								Caspar::logout();
							...
							else 
							{
								...
								$user = new TBGUser($row->get(Users::ID), $row);
				...
				elseif (Caspar::isCLI())
				{
					$row = Caspar::getB2DBInstance()->getTable('\\thebuggenie\\tables\Users')->getByUsername(TBGContext::getCurrentCLIusername());
				
				}
				// guest user
				elseif (!Settings::isLoginRequired())
				{
					$row = Caspar::getB2DBInstance()->getTable('\\thebuggenie\\tables\Users')->getByUserID(Settings::getDefaultUserID());
				}

				if ($row)
				{
					if (!$row->get(\thebuggenie\tables\Scopes::ENABLED))
					{
						throw new Exception('This account belongs to a scope that is not active');
					}
					elseif (!$row->get(Users::ACTIVATED))
					{
						throw new Exception('This account has not been activated yet');
					}
					elseif (!$row->get(Users::ENABLED))
					{
						throw new Exception('This account has been suspended');
					}
					$user = \caspar\core\Caspar::factory()->manufacture('\\thebuggenie\\core\\User', $row->get(Users::ID), $row);
					
					if ($external == false)
					{
						if ($raw == false)
						{
							\caspar\core\Caspar::getResponse()->setCookie('tbg3_password', $password);
						}
						else
						{
							\caspar\core\Caspar::getResponse()->setCookie('tbg3_password', self::hashPassword($password));
						}
		...
		public static function getUsersCount()
		{
			...
			{
				self::$_num_users = Caspar::getB2DBInstance()->getTable('\\thebuggenie\\tables\Users')->countUsers();
			}
		...
		protected function _preSave($is_new)
		{
			...
			if (!$this->_group_id)
			{
				$this->setGroup(Settings::getDefaultGroup());
			}
			if ($this->_deleted)
			{
				try
				{
					if ($this->getGroup() instanceof \thebuggenie\entities\Group)
					{
		...
		protected function _postSave($is_new)
		{
			if ($is_new)
			{
				...
				TBGDashboardViewsTable::getTable()->setDefaultViews($this->getID(), TBGDashboardView::TYPE_USER);
			}
			
			if ($this->_timezone !== null)
			{
				Settings::saveSetting('timezone', $this->_timezone, 'core', null, $this->getID());
			}
			else
			{
				Settings::saveSetting('timezone', 'sys', 'core', null, $this->getID());
			}
			
			if ($this->_language != null)
			{
				Settings::saveSetting('language', $this->_language, 'core', null, $this->getID());
			}
			else
			{
				Settings::saveSetting('language', 'sys', 'core', null, $this->getID());
			}
		}
		...
		{
			if (\caspar\core\Caspar::getUser() instanceof TBGUser)
			{
				return \caspar\core\Caspar::getUser()->isGuest();
			}
			else
			{
				return true;
		...
		{
			\caspar\core\Logging::log("User with id {$this->getID()} set up successfully");
		}
		...
		{
			return (bool) ($this->getID() == \caspar\core\Caspar::getUser()->getID());
		}
		...
		{
			$this->_userstate = Settings::getOnlineState();
		...
		{
			$this->_userstate = Settings::getOfflineState();
		...
		protected function _populateTeams()
		{
			if ($this->teams === null)
			{
				...
				\caspar\core\Logging::log('...done (Populating user teams)');
			}
		}
		...
		{
			return (bool) (!$this->isLoggedIn() || ($this->getID() == Settings::getDefaultUserID() && Settings::isDefaultUserGuest()));
		}
		...
		protected function _populateStarredIssues()
		{
			if ($this->_starredissues === null)
			{
				...
				if ($res = Caspar::getB2DBInstance()->getTable('TBGUserIssuesTable')->getUserStarredIssues($this->getID()))
				{
					while ($row = $res->getNextRow())
					{
		...
		{
			$this->_populateStarredIssues();
			\caspar\core\Logging::log("Starring issue with id {$issue_id} for user with id " . $this->getID());
			if ($this->isLoggedIn() == true && $this->isGuest() == false)
			{
				if (array_key_exists($issue_id, $this->_starredissues))
				{
					\caspar\core\Logging::log('Already starred');
					return true;
				}
				\caspar\core\Logging::log('Logged in and unstarred, continuing');
			...
			{
				\caspar\core\Logging::log('Not logged in');
				return false;
			}
		}
		...
		public function removeStarredIssue($issue_id)
		{
			...
			Caspar::getB2DBInstance()->getTable('TBGUserIssuesTable')->doDelete($crit);
		...
		protected function _setupFriends()
		{
			if ($this->_friends === null)
			{
				...
				if ($res = TBGBuddiesTable::getTable()->getFriendsByUserID($this->getID()))
				{
					while ($row = $res->getNextRow())
					{
						$this->_friends[$row->get(TBGBuddiesTable::BID)] = \caspar\core\Caspar::factory()->manufacture('\\thebuggenie\\core\\User', $row->get(TBGBuddiesTable::BID));
					}
				}
			}
		}
		...
		public function getState()
		{
			...
			if ($this->isActive())
				return Settings::getOnlineState();
			elseif ($this->isAway())
				return Settings::getAwayState();
			else
				return Settings::getOfflineState();
		}
		...
		{
			Caspar::getB2DBInstance()->getTable('TBGTeamMembersTable')->clearTeamsByUserID($this->getID());
		}
		...
		{
			Caspar::getB2DBInstance()->getTable('TBGClientMembersTable')->clearClientsByUserID($this->getID());
		}
		 ...
		 * 
		 * @param \thebuggenie\entities\Group $group 
		 */
		public function setGroup(\thebuggenie\entities\Group $group)
		{
		...
		{
			if (!Settings::isGravatarsEnabled()) return false;
		...
		{
			$res = Caspar::getB2DBInstance()->getTable('\\thebuggenie\\tables\Users')->getByDetails($details);
		...
		public static function findUsers($details, $limit = null)
		{
			...
			if ($res = Caspar::getB2DBInstance()->getTable('\\thebuggenie\\tables\Users')->getByDetails($details))
			{
				while ($row = $res->getNextRow())
				{
					$retarr[$row->get(Users::ID)] = \caspar\core\Caspar::factory()->manufacture('\\thebuggenie\\core\\User', $row->get(Users::ID), $row);
				}
			}
			return $retarr;
		}
		...
		{
			$permissive = (isset($permissive)) ? $permissive : Settings::isPermissive();
			if ($target_id === null)
			{
		...
		public function getTimezone()
		{
			if ($this->_timezone == null)
			{
				$this->_timezone = Settings::get('timezone', 'core', null, $this->getID());
			}
		...
		{
			return ($this->_language != '') ? $this->_language : Settings::getLanguage();
		}
		...
		{
			$retval = array();
			if ($res = Caspar::getB2DBInstance()->getTable('TBGIssuesTable')->getIssuesPostedByUser($this->getID(), $number))
			{
				while ($row = $res->getNextRow())
				{
		...
		public function _populateDashboardViews()
		{
			if ($this->_dashboardviews === null)
			{
				$this->_dashboardviews = TBGDashboardView::getViews($this->getID(), TBGDashboardView::TYPE_USER);
			}
		}

		public function getDashboardViews()
		{
			$this->_populateDashboardViews();
