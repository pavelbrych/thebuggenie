...
//			if ($scope === null || $scope->getID() == Context::getScope()->getID())
//				TBGContext::addAutoloaderClassPath($module_classpath);
			
			$scope_id = ($scope) ? $scope->getID() : Context::getScope()->getID();
			...
			list($classname, $version) = $details;
  			...
  			if (!Context::getScope() instanceof Scope) throw new Exception('No scope??');

			\caspar\core\Logging::log('installing module ' . $module_name);
		...
		public static function uploadModule($module_archive, $scope = null)
		{
			...
			else
			{
				...
				if (is_dir($module_basepath) === false)
				{
					...
					{
						\caspar\core\Logging::log('Try to upload module archive ' . $module_archive['name'] . ': unable to create module directory ' . $module_basepath); 
						throw new Exception('Unable to create module directory ' . $module_basepath);
					}
					if ($zip->extractTo($module_basepath) === false)
					{
						\caspar\core\Logging::log('Try to upload module archive ' . $module_archive['name'] . ': unable to extract archive into ' . $module_basepath);
		...
		final public function install($scope)
		{
			try
			{
				...
				if (Context::getScope()->isDefault() && is_dir($b2db_classpath))
				{
//					TBGContext::addAutoloaderClassPath($b2db_classpath);
		...
		{
			\caspar\core\Logging::log($message, $this->getName(), $level);
		}
		...
		public function enable()
		{
			...
			Caspar::getB2DBInstance()->getTable('Modules')->doUpdateById($crit, $this->getID());
			$this->_enabled = true;
		}
		...
		final public function uninstall($scope = null)
		{
			...
			Caspar::getB2DBInstance()->getTable('Modules')->doDeleteById($this->getID());
			Settings::deleteModuleSettings($this->getName(), $scope);
			Context::deleteModulePermissions($this->getName(), $scope);
		}
		...
		{
			$this->_availablepermissions[$permission_name] = array('description' => Caspar::getI18n()->__($description), 'target_id' => $target);
		}
		...
		{
			$scope = ($scope === null) ? Context::getScope()->getID() : $scope;
			Caspar::getB2DBInstance()->getTable('TBGModulePermissionsTable')->deleteByModuleAndUIDandGIDandTIDandScope($this->getName(), $uid, $gid, $tid, $scope);
			Caspar::getB2DBInstance()->getTable('TBGModulePermissionsTable')->setPermissionByModuleAndUIDandGIDandTIDandScope($this->getName(), $uid, $gid, $tid, $allowed, $scope);
		...
		{
			return Settings::get($setting, $this->getName(), Context::getScope()->getID(), $uid);
		}
		
		public function saveSetting($setting, $value, $uid = 0, $scope = null)
		{
			$scope = ($scope === null) ? Context::getScope()->getID() : $scope;
			return Settings::saveSetting($setting, $value, $this->getName(), $scope, $uid);
		}
		
		public function deleteSetting($setting, $uid = null, $scope = null)
		{
			return Settings::deleteSetting($setting, $this->getName(), $scope, $uid);
		}
		...
		public static function getAllModulePermissions($module, $uid, $tid, $gid)
		{
			...
			//$sql .= " AND b2mp.scope = " . Context::getScope()->getID();
			$crit->addWhere(TBGModulePermissionsTable::SCOPE, Context::getScope()->getID());
			...
			$permissions = array();
			$res = Caspar::getB2DBInstance()->getTable('TBGModulePermissionsTable')->doSelect($crit);
		...
		{
			return Caspar::getI18n()->__($this->_account_settings_name);
		}
