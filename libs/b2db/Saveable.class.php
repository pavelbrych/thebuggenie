	...
	class Saveable
	{
		...
		protected function _getPopulatedObjectFromProperty($property)
		{
			if (is_numeric($this->$property) && $this->$property > 0)
			{
				...
				{
					$this->$property = \caspar\core\Caspar::factory()->manufacture($type_name, $this->$property);
				}
				else
				{
		...
		{
			Logging::log('Populating ' . get_class($this) . ' with id ' . $this->_id, 'B2DB');
			...
			foreach ($this->getB2DBTable()->getColumns() as $column)
			{
				...
				switch ($property_type)
				{
					case 'class':
						...
						Logging::log('Populating foreign object of type ' . $type_name . ' with value ' . $value . ' for property ' . $property_name, 'B2DB');
						//if (!$row->get($column))
			...
			Logging::log('Done populating ' . get_class($this) . ' with id ' . $this->_id, 'B2DB');
		}
